        ...
        <div v-if="isShow">
          <a-form-item
            ...
            <a-select v-decorator="['userId', validatorRules.userId]"
                      ...
                      placeholder="请选择指派用户手机号">
              <a-select-option v-for="(user,index) in userList" :key="index.toString()"
                               :value="user.id">
                {{user.mobile}}
              </a-select-option>
            </a-select>
...
<script>
  ...
  import {getCouponList, getCouponNumber, getUList} from '@/api/api'

  export default {
    name: "CouponAssignModal",
    data() {
      return {
        title: "操作",
        visible: false,
        ...
        validatorRules: {
          userId: {rules: [{required: true, message: '请选择指派用户手机号..'}]},
